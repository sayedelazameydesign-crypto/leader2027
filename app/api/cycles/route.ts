...
import { getSettings, createCycle } from "@/lib/domain/settings/service";
...
export async function GET(req: Request) {
  ...
  return result.ok
    ? NextResponse.json({ cycles: result.value.cycles })
    : failResponse(result);
}
...
export async function POST(req: Request) {
  ...
  const result = createCycle(actor, body, repos);
  return result.ok
    ? NextResponse.json({ cycle: result.value }, { status: 201 })
    : failResponse(result);
}

...
import { getSettings, createRegion } from "@/lib/domain/settings/service";
...
export async function GET(req: Request) {
  ...
  return result.ok
    ? NextResponse.json({ regions: result.value.regions })
    : failResponse(result);
}
...
export async function POST(req: Request) {
  ...
  const result = createRegion(actor, body, repos);
  return result.ok
    ? NextResponse.json({ region: result.value }, { status: 201 })
    : failResponse(result);
}
